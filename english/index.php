<?php include 'includesEN/header.php'?>
			...
			<ul>
				<li>
					...
					<div class="admin-panel-content">
						<h3 class="panel-title">Join Us on Facebook</h3>
   ...
   <div class="flipbox_area top_feature">
	   <div class="container">
		   <div class="row nagative_margin">
                                    <?php
                   $stmt = $DB->prepare("SELECT tituloMenuEn,descricaoMenuEn,iconMenu FROM menusgiratorios WHERE 1 LIMIT 4");
                   $stmt->execute();
...
echo $interval->format('%y YEARS OF EXPERIENCE');?></h6>
					   </div>
					   ...
					   <div class="section_content_text pt-4">
						   <p>Shore is an IT consulting company, with an approach focused on our employees and on the satisfaction of its partners. We always present two points that complement our line of action:</p>
					   </div>
				   </div>
				   <div class="singel_about_left mb-30">
					   <div class="singel_about_left_inner mb-3">
						   ...
						   <div class="singel-about-content">
							   <h5>Flexibility</h5>
							   <p>Flexibility is one of the main pillars of our company and allows us to adapt quickly to the needs of our employees and partners. This means that we are always ready to change and evolve according to the needs of each project.</p>
						   </div>
					   </div>
					   <div class="singel_about_left_inner">
						   ...
						   <div class="singel-about-content">
							   <h5>Speed</h5>
							   <p>Speed is another key point of our approach. We are able to deliver viable and sustainable solutions quickly and efficiently so that our employees and partners can continue to grow and develop.</p>
						   </div>
					   </div>
				   </div>
			   </div>
   ...
   <div class="flipbox_area pt-85 pb-70" style="background-image:url(assets/img/slider/slider-4.jpg)";>
	   <div class="container">
		   ...
		   <div class="row">
                           <?php
                   $stmt = $DB->prepare("SELECT nomeServicoEn,detalhesServicoEn,icon FROM servicos WHERE 1 LIMIT 6");
                   $stmt->execute();
   ...
   <div class="service_area bg_color2 pt-85 pb-75">
		<div class="container">
			...
			<div class="row">
						...
						<?php
							$stmt = $DB->prepare("SELECT id,empregoEn,descritivoEn,requisitosEn FROM oportunidades WHERE 1 ORDER BY data DESC LIMIT 6");
							$stmt->execute();
							...
							while ($stmt->fetch())
								...
								<div class='service_style_one text_center pt-40 pb-40 pl-3 pr-3 mb-4'>
									...
									<div class='service_style_one_text'>
										<p>".($descritivo == NULL ? "Requirements: " .substr($requisitos,0,120) : substr($descritivo,0,120))."</p>
									</div>
									<div class='service_style_one_button pt-3'>
									<form action='opportunity' method='GET' id='ff'>
									<input name='idEmprego' id='idEmprego' type='hidden' value='$id'>
									<div class='service_style_one_button pt-3'>
									<a><input type='submit' value='Read More' style='padding: 0; border: none; background: none;'> <i class='fa fa-long-arrow-right'></i></a>
									</div>
									</form>
									</div>
								</div>
				...
				<div class="col-lg-12 col-md-12 col-sm-12">
						<div class="button two" style='text-align:center;'>
							<a href="opportunities">View more</a>
						</div>
					</div>
				</div>
			</div>
		   ...
		   <div class="row">
			   <div class="col-lg-12" style='text-align: center;'>
				   <div class="section_title white text_left mb-60 mt-3">
					   ...
					   <div class="section_main_title">
						   <h1>We look for talent, honesty and delivery.</h1>
					   </div>
				   </div>
			   </div>
			   <div class="col-lg-3">
   ...
   <!--==================================================-->
				...
				<div class="col-lg-6">
					<div class="section_title text_left mb-50 mt-3">
						...
						<div class="section_title_text pt-2">
							<p>Shore is an IT consulting company, with an approach focused on our employees and on the satisfaction of its partners.</p>
						</div>
						<div class="em_bar">
							<div class="em_bar_bg"></div>
						</div>
				...
				<div class="col-lg-6">
					<div class="contact_from">
						<div class="contact_from_box">
							...
							<form action="phpcodes/contacto.php" method="POST">
								<div class="row">
									<div class="col-lg-12">
									...
									<div class="col-lg-12">
										<div class="form_box mb-30">
											<textarea name="message" id="message" cols="30" rows="10" placeholder="Message" style="resize: none;" required></textarea>
										</div>
										<div class="quote_btn">
											<button class="btn" type="submit" name='submitEn' id='submitEn'>Submit</button>
										</div>
									</div>
								</div>
							</form>
	...
	<div class="blog_area bg_color2 pt-85 pb-75">
		<div class="container">
			...
			<div class="row">
										<?php
						$stmt = $DB->prepare("SELECT idNoticia,noticiaEn,descNoticiaEn,data,imagem FROM noticias WHERE 1 ORDER BY data DESC LIMIT 3");
						$stmt->execute();
...
<?php
    if(isset($_SESSION['candidaturaEnviadaEn'])) {
  echo "<script>Swal.fire(
  'Application submitted successfully!',
  '',
  'success'
) </script>";
  unset($_SESSION['candidaturaEnviadaEn']);
}
    if(isset($_SESSION['contactoEnviadoEn'])) {
  echo "<script>Swal.fire(
  'Message sent successfully!',
  '',
  'success'
) </script>";
  unset($_SESSION['contactoEnviadoEn']);
}
?>
